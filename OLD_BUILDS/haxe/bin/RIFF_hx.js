...
var brian151_riff_Section = $hx_exports.brian151.riff.Section = function(src,offset,len,id) {
	...
	var tempview = new DataView(src,offset + 8,len);
	this.view = new DataView(buf);
	var _g = 0;
	while(_g < len) {
		var i = _g++;
		var tempByte = tempview.getUint8(i);
		this.view.setUint8(i,tempByte);
	}
	this.length = len;
...
brian151_riff_Section.prototype = {
	...
	,getString: function(type,len,offset) {
		var out = "";
		if(type == "ASCII") {
			var _g = 0;
			while(_g < len) {
				...
				if(offset2 < this.length) {
					var charByte = this.view.getUint8(offset2);
					out += String.fromCharCode(charByte);
				} else throw new RangeError("specified string exceeds length of section!");
			}
		} else if(type == "FOURCC") out += this.getString("ASCII",4,offset);
		return out;
	}
};
